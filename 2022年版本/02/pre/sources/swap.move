module pre::swap {
    ...
    public entry fun add_pool<CoinX, CoinY>(pool: &mut Pool<CoinX, CoinY>,
        ...
        transfer::transfer(coin_x, sender(ctx));
        transfer::transfer(coin_y, sender(ctx));
    }
    ...
    public entry fun remove<CoinX, CoinY>(pool: &mut Pool<CoinX, CoinY>,
        ...
        transfer::transfer(x, sender(ctx));
        transfer::transfer(y, sender(ctx));
    ...
    public entry fun swap_x_y<CoinX, CoinY>(pool: &mut Pool<CoinX, CoinY>,
                                            ...
                                            amount: u64,
                                            ctx: &mut TxContext) {
        let coin_in = coin::zero<CoinX>(ctx);
        ...
        transfer::transfer(y, sender(ctx));
        transfer::transfer(coin_in, sender(ctx));
    }
    ...
    public entry fun swap_y_x<CoinX, CoinY>(pool: &mut Pool<CoinX, CoinY>,
                                            ...
                                            amount: u64,
                                            ctx: &mut TxContext) {
        let coin_in = coin::zero<CoinY>(ctx);
        pay::join_vec(&mut coin_in, in);
